...
class Population:
    ...
    def __init__(
        ...
        self.selection_func = ga_selection
    ...
    def procreation(self):
        self.children = self.procreation_func(self.survivors)
        ...
        for generation in range(self.max_generations):
            ...
            if generation % 10 == 0:
                img_shortest_route = create_cities_img(
                    self.fittest_life, human_injection=False, generation_no=generation
                )
                cv2.imwrite(f"generation_{generation}.png", img_shortest_route)
                generation_images.append(img_shortest_route)

        # Create a GIF from the list of images
        file_name = str(self.fittest_life.fitness_proxy).replace(".", "-")
        imageio.mimsave(
            f"{file_name}.gif", generation_images, duration=0.5
